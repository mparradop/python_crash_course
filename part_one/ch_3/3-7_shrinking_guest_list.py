...
print(f"\nYou won't believe it guys...there's now only a table for three, so you will receive another invitation for another day")
current_lenght = len(guests)
print(current_lenght)

for i in range(2,current_lenght):
    guest_removed=guests.pop()
    print(f'{guest_removed}, sorry this time I cannot invite you for dinner')
print(f'Table for three?:\n{guests}')#print a message to each of the two people on your list

#empty the list
current_lenght = len(guests)
print(current_lenght)
while current_lenght!=0:
    del guests[-1]#delete the last item in the list
    current_lenght = len(guests)
    print(current_lenght)

print(f'The event has been canceled. The list is empty {guests}')